...
for p_index in range(len(p)):
    for count in range(n_max):
        ...
        hamming_weight = get_wieght(binary_array)
        if hamming_weight <= lmin:
            continue
        elif hamming_weight >= lmax:
            success_sum += 1
            continue
        elif has_path(list2dict(subgraph_edges), start_node - 1, finish_node - 1):
            success_sum += 1
...
plt.figure()
plt.title('pr{' + str (start_node) + ',' + str(finish_node) + '} (p) for (lmin = ' + str(lmin) + ', lmax = ' + str(lmax) + ')')
plt.plot(p, pr)
plt.show()
